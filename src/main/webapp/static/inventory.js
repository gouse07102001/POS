...
function addInventory() {
	...
	var product = $("#inputProduct option:selected").text();
	var json = toJson($form);
...
function addProduct2(data) {
	$("#inputProduct").empty();
	document.getElementById("add-inventory-form").reset();
	var select = document.getElementById('inputProduct');
	var opt = document.createElement('option');
	opt.value = "none";
	opt.innerHTML = "Select a Barcode";
	...
	for (var i in data) {
		...
		opt.innerHTML = p.barcode;
		select.appendChild(opt);
	}
}
function changeInventory(data) {
	$("#inventory-edit-form input[name=quantity]").val(
		data.quantity);
	$('#edit-inventory').modal('toggle');
}
...
function getInventory(id) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (data == null)
				data.quantity = 0;
			popup.innerHTML = data.quantity;
			popup.classList.toggle("show");
			var inv = document.getElementById("inv" + id);

			if (data.quantity <= 20)
				inv.style.backgroundColor = "red";
			else if (data.quantity > 20 && data.quantity <= 110)
				inv.style.backgroundColor = "orange";
			else inv.style.backgroundColor = "green";
			return data; // ...
		},
		error: function(response) {
			var reponseMessage = JSON.parse(response.responseText).message;
...
function fileValidation() {
	var fileInput = document.getElementById('inventoryFile');
...
function processDataInventory() {
	...
	if (fileValidation())
		...
		$('#MyModal').on('hidden.bs.modal', function() {
			$(this).find('upload-inventory-modal').trigger('reset');
		})
	}
}
...
function addProductNames(i, data) {
	if (i == data.length) {
		displayInventoryList(data);
		return true;
	}
	...
	$.ajax({
		...
		success: function(pojo) {
			data[i].name = pojo.productName;
			data[i].mrp = pojo.mrp;
			addProductNames(i + 1, data);
		},
		error: function(response) {
			var reponseMessage = JSON.parse(response.responseText).message;
			errorDisplay('danger', reponseMessage);
...
function displayInventory(data, id) {
	$("#editProduct").empty();
	...
	$("#edit-inventory-form input[name=editProduct]").val(
		data.barcode);
	$("#edit-inventory-form input[name=quantity]").val(data.quantity);
	$('#edit-inventory-modal').modal('toggle');
}
