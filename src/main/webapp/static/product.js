...
function getBrandCategory(i, data) {
	if (i == data.length) {
		displayProductList(data);
		return true;
	}
	var url = getBrandUrl() + "s/" + data[i].brandCategory;
	$.ajax({
		url: url,
		type: 'GET',
		success: function(brands) {
			data[i].brand = brands.brand;
			data[i].category = brands.category;
			getBrandCategory(i + 1, data);
		},
		error: function(response) {
			var reponseMessage = JSON.parse(response.responseText).message;
			errorDisplay('danger', reponseMessage);
...
function addProduct(event) {
	...
	$.ajax({
		...
		success: function(response) {
			...
			errorDisplay('success', 'Product successfully added.');
			getProductList(); // ...
		},
		error: function(response) {
			var reponseMessage = JSON.parse(response.responseText).message;
...
function displayProduct(data, id) {
	...
	$("#product-edit-form input[name=productId]").val(id);
	...
	editBrandCategory(data.brandCategory);
	$('#edit-product-modal').modal('toggle');
}
...
function fileValidation() {
	...
	var allowedExtensions = /(\.tsv)$/i;
	if (!allowedExtensions.exec(filePath)) {
		errorDisplay('danger', 'Please upload file having extension .tsv');
		fileInput.value = '';
		return false;
	} else {
		//Image preview
...
function processData() {
	...
	if (fileValidation())
		...
		$('#MyModal').on('hidden.bs.modal', function() {
			$(this).find('upload-product-modal').trigger('reset');
		})
	}
}
...
function searchByBarcode() {
	...
	var url = getProductUrl() + "s/search/" + barcode;
	...
	$.ajax({
		...
		type: 'GET',
		success: function(data) {
			displayEditProduct(data.productId); // ...
		},
		error: function(response) {
...
function init() {
	...
	$('#product-form').on('submit', e => e.preventDefault())
	$('#product-edit-form').on('submit', e => e.preventDefault())
...
function addBC(data) {
	var select = document.getElementById('inputBC');
	for (var i in data) {
		var e = data[i];
...
function editBrandCategory(brandId) {
	...
	$.ajax({
		...
		type: 'GET',
		success: function(data) {
			getBrandCategoryDetails(data, brandId); // ...
		},
		error: function(response) {
			var reponseMessage = JSON.parse(response.responseText).message;
			errorDisplay('danger', reponseMessage);
...
function getBrandCategoryDetails(datafull, brandId) {
	let url = getBrandUrl() + "s/" + brandId;
	$.ajax({
		url: url,
		type: 'GET',
		success: function(data) {
...
function makeDropDown(data, brandObj) {
	var select = document.getElementById('editBrandCategory');
	var first = document.createElement('option');
	first.value = brandObj.brandId;
	first.innerHTML = brandObj.brand + "-" + brandObj.category;
	select.appendChild(first);
	for (var i in data) {
		var e = data[i];
		if ((e.brand === brandObj.brand) && (e.category === brandObj.category)) {
			continue
		}
		var opt = document.createElement('option');
		opt.value = e.brandId;
...
function highlight() {
	document.getElementById("product-link").style.fontWeight = 900;
...
$(document).ready(addBrandCategory);
...
function getBarcodes() {
	var product_url = "/pos/api/brand-category"
	$.ajax({
		url: product_url,
		type: 'GET',
		success: function(values) {
			...
			label.innerHTML = "Select a Brand_Category";
			label.htmlFor = "Brand_Category";
		},
		error: function(response) {
			var reponseMessage = JSON.parse(response.responseText).message;
			errorDisplay('danger', reponseMessage);
...
/*
 * @license Papa Parse v5.0.2 https://github.com/mholt/PapaParse License: MIT
 */
