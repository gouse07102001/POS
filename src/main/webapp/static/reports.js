...
function getBrandList(){
	...
	$.ajax({
	   ...
	   success: function(data) {
	   		displayUniqueBrandsCategories(data);  
	   },
...
function getSalesReport(){
	var brandCategory = $('#select-brand-category').find(":selected").val();
	var index = brandCategory.indexOf("-")
	var brand = brandCategory.substring(0,index)
	var category = brandCategory.substring(index+1,brandCategory.length)
	var startDate = $('#inputStartDate').val();
	var endDate = $('#inputEndDate').val();

	var data = JSON.stringify({ "brand": brand, "category": category, "startDate": startDate, "endDate": endDate });
	...
	$.ajax({
	   ...
	   success: function(data) {
	   		...
	   		just();
	   		if(data.length == 0){
				errorDisplay('message', "No sales happend in the given timeline");
				return false
	}
	   		printSalesReport(data);
	   		errorDisplay('success', "Sales Report Generated SuccessFully");
	   },
	   error: function(response) {
	   	var reponseMessage = JSON.parse(response.responseText).message;
	   	errorDisplay('danger', reponseMessage);
...
function getInventoryReport(){
	...
	$.ajax({
	   ...
	   success: function(data) {
	   		...
	   		errorDisplay('success', "Inventory Report Generated SuccessFully");  
	   },
	   error: function(response) {
	   	var reponseMessage = JSON.parse(response.responseText).message;
	   	errorDisplay('danger', reponseMessage);
...
function displayUniqueBrandsCategories(data){
	var $selectBrandCategory = $('#select-brand-category');
	...
	for(var i in data){
		...
		brandCategory.push(brands[i] +"-"+ categories[i])
	}
	
	

	var unique_brands_categories = new Set(brandCategory);

	unique_brands_categories = Array.from(unique_brands_categories);
	console.log(unique_brands_categories)

	for(var i in unique_brands_categories){
		var option = `<option value='${unique_brands_categories[i]}'>${unique_brands_categories[i]}</option>`;
        $selectBrandCategory.append(option);
	}

}

function printBrandReport(arr){
	var config = {
		quoteChar: '',
		escapeChar: '',
		delimiter: "\t"
...
function printInventoryReport(arr){
	var config = {
		quoteChar: '',
		escapeChar: '',
		delimiter: "\t"
...
function highlight()
{
	document.getElementById("reports-link").style.fontWeight = 900;
