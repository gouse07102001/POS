...
function getSalesReport() {
	...
	var category = $('#select-category').find(":selected").val();
	...
	$.ajax({
		...
		success: function(data) {
			...
			printSalesReport(data,brand,category,startDate,endDate);
...
function displayUniqueBrandsCategories(data) {
	...
	var unique_brands = new Set(brands);
	var unique_categories = new Set(categories);
	unique_brands = Array.from(unique_brands);
	unique_categories = Array.from(unique_categories);
	for (var i in unique_brands) {
		var option = `<option value='${unique_brands[i]}'>${unique_brands[i]}</option>`;
		$selectBrand.append(option);
	}
	for (var i in unique_categories) {
		var option = `<option value='${unique_categories[i]}'>${unique_categories[i]}</option>`;
...
function printSalesReport(arr,brand,category,startDate,endDate) {
	var config = {
		quoteChar: '',
		escapeChar: '',
		delimiter: "\t"
	...
	var tempLink = document.createElement('a');
	tempLink.href = fileUrl;
	tempLink.setAttribute('download', 'SalesReport from '+startDate+' to '+endDate+' .tsv');
	tempLink.click();
}
